...
library(ggplot2)
p <- ggplot(data = marbindrd11, aes(x = `rownames(marbindrd)`, y = tapply.marballrd.Freq..list.marballrd.Var2...mean.)) + geom_point() + geom_errorbar(aes(ymin = tapply.marballrd.Freq..list.marballrd.Var2...mean. - sd, ymax = tapply.marballrd.Freq..list.marballrd.Var2...mean. + sd), width = .2)
print(p)
p + labs(title = "Marbles Mean Number of Visits Per Day Given All Collared Sheep")

library(ggplot2)
marbvisitplot <- ggplot(data = marbindsheepvisitperday1, aes(x = `rownames(marbindsheepvisitperday)`, y = tapply.marballsheepvisitsday.Freq..list.marballsheepvisitsday.Var2...)) +
